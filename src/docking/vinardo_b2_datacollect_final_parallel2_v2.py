...
# --- CONFIGURATION ---
# ======================================================================
MOL2_DIR = "/home/kumara/malakafahs/dock/allosteric/vinardo/poses_mol2"
LOG_DIR = "/home/kumara/malakafahs/dock/allosteric/vinardo/pose_logs"
LSALIGN_EXE = "./LSalign"
REFERENCE_MOL2 = "4tq_reference.mol2"
OUTPUT_CSV = "b2_vinardo_results_v2.csv"
NUM_CORES = 16
MOLECULE_LIMIT = 0
FP_BITS = 2048
FP_RADIUS = 2
...
def calculate_com_distance_from_mol2(mol2_file, ref_mol):
    """Backup method: compute COM distance directly from raw mol2 file."""
    try:
...
def run_lsalign_and_parse(ligand_mol2, reference_mol2, lsalign_exe):
    ...
    try:
        ...
        data_line = None
        for i, line in enumerate(stdout_output.splitlines()):
            if line.strip().startswith("QEURY_NAME") or line.strip().startswith("QUERY_NAME"):
                for j in range(i + 2, len(stdout_output.splitlines())):
                    potential = stdout_output.splitlines()[j].strip()
...
def generate_ecfp_from_mol(mol, fp_bits, fp_radius):
    if mol is None:
        return pd.Series([np.nan] * fp_bits, index=[f'ECFP_{i}' for i in range(fp_bits)])
    try:
...
def worker_process_molecule(args):
    ...
    # Compute COM distance (with mol2 fallback)
    ...
    row.update(run_lsalign_and_parse(ligand_mol2, REFERENCE_MOL2_PATH, LSALIGN_EXE))
    return row
...
if __name__ == "__main__":
    try:
        subprocess.run([LSALIGN_EXE, "-h"], capture_output=True)
